...
namespace RagNext.Designer.Avalonia.Services
{
    ...
    {
        public static string[] Validate(Game game)
        {
            var errors = new List<string>();
            if (game == null)
            {
                errors.Add("Error: Game instance is null.");
                return errors.ToArray();
            }
            if (string.IsNullOrWhiteSpace(game.Title))
                errors.Add("Error: Game title is required.");
            if (string.IsNullOrWhiteSpace(game.Author))
                errors.Add("Warning: Game author name is empty.");
            if (game.Rooms == null || game.Rooms.Count == 0)
                errors.Add("Error: The game must have at least one room.");
            else
            {
                foreach (var r in game.Rooms)
                {
                    if (string.IsNullOrWhiteSpace(r.Name))
                        errors.Add($"Error: Room with ID '{r.Id}' has an empty or white-space name.");
                }
            }
            ...
            if (game.Rooms != null)
            {
                foreach (var r in game.Rooms)
                {
                    ...
                    {
                        if (!game.Rooms.Any(other => other.Id == exit.Value))
                        {
                            errors.Add($"Error: Room '{r.Name}' has exit '{exit.Key}' pointing to invalid room ID '{exit.Value}'.");
                        }
                    }
                }
            }
