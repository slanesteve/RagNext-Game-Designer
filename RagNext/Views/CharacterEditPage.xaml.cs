...
namespace RagNext.Views
{
    ...
    public partial class CharacterEditPage : ContentPage
    {
        ...
        private async System.Threading.Tasks.Task SetCharacterAsync(string? value)
        {
            ...
            UpdatePortraitImage(ch.PortraitImagePath);
            ch.PropertyChanged += (s, e) =>
            {
                if (e.PropertyName == nameof(Character.PortraitImagePath))
                    UpdatePortraitImage(ch.PortraitImagePath);
            };
        }
        ...
        private async void OnPortraitDrop(object sender, DropEventArgs e)
        {
            ...
            if (!string.IsNullOrWhiteSpace(localPath))
            {
                ch.PortraitImagePath = localPath;
        ...
        {
            if (BindingContext is Character ch)
            {
                ch.PortraitImagePath = null;
        ...
        {
            if (BindingContext is not Character ch)
            {
                await DisplayAlert("Portrait", "No character bound.", "OK");
                return;
            }
            ...
            try
            {
                ...
                if (mediaLibrary is not null && treeStore is not null)
                {
                    ...
                    // Add to "Characters" media folder in media tree
