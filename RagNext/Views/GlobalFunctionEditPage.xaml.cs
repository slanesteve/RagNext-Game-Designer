...
{
    [QueryProperty(nameof(FunctionId), "functionId")]
    public partial class GlobalFunctionEditPage : ContentPage
    {
        public string? FunctionId { set { _ = SetFunctionAsync(value); } }

        // A collection containing only this GlobalFunction (which inherits from Action)
        // so we can bind it directly to the ActionTreeView's generic Actions property!
        public ObservableCollection<RagsCore.Models.Action> SingleFunctionCollection { get; } = new();

        private readonly IAIChatService? _ai;

        public GlobalFunctionEditPage()
        {
            InitializeComponent();
        ...
        private async System.Threading.Tasks.Task SetFunctionAsync(string? value)
        {
            if (string.IsNullOrWhiteSpace(value)) return;
            if (!Guid.TryParse(value, out var id)) return;

            var game = App.CurrentGame;
            var func = game?.Functions?.FirstOrDefault(f => f.Id == id);
            if (func is null)
            {
                await DisplayAlert("Not found", "Function not found in current game.", "OK");
        ...
        private async void OnAskAIClicked(object sender, EventArgs e)
        {
            ...
            {
                prompt = $"Suggest a short, clean, descriptive C#/script function name for: {entry.Text}";
            }
