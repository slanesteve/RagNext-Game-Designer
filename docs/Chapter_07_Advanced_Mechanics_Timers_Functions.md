...
In Studio, build a poison damage system:

1. Create variable `PlayerHP` = `100` and boolean `IsPoisoned` = `True`.
2. Go to **⚙️ Variables & Timers** and create a timer `PoisonTickTimer` (Interval = `5.0` seconds).
3. Open **🎨 Visual Graph Editor** on `PoisonTickTimer`:
   - Condition: `IsPoisoned == True`
     - **True**: `Modify Integer: PlayerHP -= 5`, `Play Sound: poison_drip.wav`, `Print Message ("Poison burns in your veins! (-5 HP)")`.
4. Save your game!
